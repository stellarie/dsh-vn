...
- **没有随附配置列出它。** 挂载面是配置的 `dsh.profile.bundles` 列表加上一条依赖声明，因为裸行在未声明时无法解析。正在使用的 `imouto-yuu` 配置属于本仓库之外的用户状态，并且链接的是另一份检出。
...
- **未针对实时驱动验证。** 读取器与面板在合成的状态目录和打桩的路由上测试。两者都未在浏览器会话中针对实时驱动状态目录运行过。
