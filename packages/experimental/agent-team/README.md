...
`TeamService` owns the generated `agentTeams/view`, `agentTeams/createTask`, `agentTeams/updateTask`, and `agentTeams/sendMessage` Remote methods beside the roster, mailbox, task, and lifecycle operations. `agentTeams/sendMessage` admits one browser steering line: its request carries a target name and one text line, and the Host frames that line as one text content block for the same durable mailbox `sendMessage()` uses. Its first parameter is the addressed member, so a browser caller that names the Team Lead session steers as the Lead. The `./remote` export supplies the Client contribution mounted by the Web UI, while `./client` re-exports the request, view, peer-message result, and task-mutation result types that are safe in a browser compilation face. Typert retains transport failures in its outer `RemoteResult`; create and update rejections remain explicit domain results inside a successful transport response, with stale update revisions distinguished as task conflicts, while a rejected steering call throws and reaches the caller as a transport failure.
