...
/** Reason of one aborted signal as an Error the caller can log. */
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (
            ...
            json(res, 415, { error: 'unsupported content type' }); return
          }
          try {
        ...
        : [
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              if (!(await authorize(req, res))) return
              if (req.method === 'GET')
              {  json(res, 200, imageTools.snapshot()); return }
              if (req.method !== 'POST')
              {  json(res, 405, { error: 'method not allowed' }); return }
              try {
                json(
                ...
                json(res, 400, { error: safeMessage(error) }); return
              }
            },
          }),
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              if (!(await authorize(req, res))) return
              if (req.method === 'GET')
              {  json(res, 200, imageTools.responseApiSnapshot()); return }
              if (req.method !== 'POST')
              {  json(res, 405, { error: 'method not allowed' }); return }
              try {
                json(
          ...
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              if (!(await authorize(req, res))) return
              if (req.method === 'GET')
              {  json(res, 200, imageTools.contextWindowSnapshot()); return }
              if (req.method !== 'POST')
              {  json(res, 405, { error: 'method not allowed' }); return }
              try {
                json(
          ...
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              if (!(await authorize(req, res))) return
              if (req.method === 'GET')
              {  json(res, 200, imageTools.modelCatalogSnapshot()); return }
              if (req.method !== 'POST')
              {  json(res, 405, { error: 'method not allowed' }); return }
              try {
                json(
          ...
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              if (!(await authorize(req, res))) return
              if (req.method === 'GET')
              {  json(res, 200, imageTools.fastModeSnapshot()); return }
              if (req.method !== 'POST')
              {  json(res, 405, { error: 'method not allowed' }); return }
              try {
                json(
          ...
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              if (!(await authorize(req, res))) return
              if (req.method === 'GET')
              {  json(res, 200, imageTools.modelFallbackSnapshot()); return }
              if (req.method !== 'POST')
              {  json(res, 405, { error: 'method not allowed' }); return }
              try {
                json(
        ...
        : [
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              if (!(await authorize(req, res))) return
              if (req.method === 'GET') {
                json(res, 200, proxySettings.proxyPreferences()); return
              }
              if (req.method !== 'POST') {
                json(res, 405, { error: 'method not allowed' }); return
              }
