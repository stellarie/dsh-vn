...
/** Absolute URL of one fetch argument. */
const requestUrl = (input: string | URL | Request): string =>
  typeof input === 'string' ? input : input instanceof URL ? input.href : input.url

/** JSON body text of one fetch request, empty when the body carries none. */
...
describe('OpenAI Codex settings model catalog', () => {
  it('renders model toggles and persists the provider-ordered visible subset', async () => {
    ...
    const fetchMock = vi.fn(
      async (
        ...
        if (path.endsWith('/fast-mode-default')) {
          if (init?.method === 'POST') {
            const patch = JSON.parse(requestBody(init)) as Partial<{
        ...
        if (path.endsWith('/proxy')) {
          if (init?.method === 'POST') {
            proxy = {
              ...proxy,
              ...(JSON.parse(requestBody(init)) as Partial<typeof proxy>),
            }
          }
          return json(proxy)
        }
        ...
        if (path.endsWith('/models')) {
          if (init?.method === 'POST')
            selected = (JSON.parse(requestBody(init)) as { models: string[] })
              .models
    ...
    const modelPost = fetchMock.mock.calls.find(
      ([input, init]) =>
        requestUrl(input).endsWith('/models') && init?.method === 'POST',
    )
    expect(modelPost).toBeDefined()
    expect(JSON.parse(requestBody(modelPost?.[1]))).toEqual({
      models: ['gpt-5.6-sol'],
    })
    ...
    const contextPosts = () =>
      fetchMock.mock.calls.filter(
        ([input, init]) =>
          requestUrl(input).endsWith('/context-window') && init?.method === 'POST',
      )
    ...
    const proxyPosts = fetchMock.mock.calls.filter(
      ([input, init]) =>
        requestUrl(input).endsWith('/proxy') && init?.method === 'POST',
    )
    ...
    const fastModePost = fetchMock.mock.calls.find(
      ([input, init]) =>
        requestUrl(input).endsWith('/fast-mode-default') &&
        init?.method === 'POST',
    )
    expect(JSON.parse(requestBody(fastModePost?.[1]))).toEqual({
    ...
    const fallbackPost = fetchMock.mock.calls.find(
      ([input, init]) =>
        requestUrl(input).endsWith('/model-fallback') && init?.method === 'POST',
    )
    expect(JSON.parse(requestBody(fallbackPost?.[1]))).toEqual({
