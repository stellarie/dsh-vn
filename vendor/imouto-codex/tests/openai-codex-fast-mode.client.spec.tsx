  ...
  it('loads the current session state, toggles only that session, and exposes aria/title semantics', async () => {
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
      if (init?.method === 'POST') {
        ...
        expect(JSON.parse(requestBody(init))).toEqual({ sessionId: 'session-a', enabled: true })
        return json({ enabled: true })
      }
      expect(requestUrl(input)).toContain(`${OPENAI_CODEX_FAST_MODE_PATH}?sessionId=session-a`)
